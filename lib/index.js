...
function messagePreview(message, max = 120) {
  const chunk = Array.isArray(message?.content)
    ? message.content.find((part) => part?.type === "text" && typeof part.text === "string")?.text
    : undefined;
  const text = typeof chunk === "string" ? chunk.replace(/\s+/g, " ").trim() : "";
...
function registerSessionMessageTools(rootCtx, toolCtx, agent, framing) {
  ...
  try {
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      description: "Inspect another session's pending message queue: the two ordered inbox lists (next-turn = messages queued for its next turn, next-step = urgent messages injected into its current step) plus the agent status and pending count. Use it before assigning work to decide whether to queue normally or jump with priority=immediate. / 查看其他会话要排队处理的消息：两条有序收件箱队列（next-turn = 排到下一轮的普通消息，next-step = 插进当前步骤的紧急消息），附 agent 状态与待处理总数。派活前先用它判断该正常排队还是用 priority=immediate 插队。",
      parameters: {
        target_session: {
          type: "string",
          required: true,
