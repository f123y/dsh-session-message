...
function registerSessionMessageTools(rootCtx, toolCtx, agent, framing) {
  ...
  try {
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      description: "Create a new session and start an agent on it. The new session appears in the session list, inherits the caller's agent preset (so it has the full toolset), and can receive messages immediately. Optionally deliver a first message to the new session. / 创建一个新会话并在其上启动一个 agent。新会话会出现在会话列表中，继承调用方的 agent 预设（拥有完整工具集），立即可接收消息。可选：同时向新会话发送一条首条消息。",
      parameters: {
        first_message: {
          type: "string",
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          let handle;
          // agentPresets 服务存在时，继承调用方 agent 的预设并 mount（挂载完整工具集）
          const presets = rootCtx.get("agentPresets");
          if (presets !== undefined) {
            const requestedPreset = agent.session.header.agentPreset;
