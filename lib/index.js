...
function registerSessionMessageTools(rootCtx, toolCtx, agent, framing) {
  ...
  try {
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      description: "Send a message to another live session. The target session's agent receives the message as a new user message and will process it in its next turn. Use session_message_list to discover live session ids first.",
      parameters: {
        target_session: {
          type: "string",
          required: true,
          description: "Exact session id of the receiving session, e.g. session-2."
        },
        content: {
          type: "string",
          required: true,
      ...
      presentCall: (args) => present("Send cross-session message", "other", args.target_session)
    })));

    disposers.push(toolCtx.tools.register(defineTool({
      name: "session_message_list",
      description: "List every live session that can receive cross-session messages: exact session id, optional title, agent status (idle/running when an agent is live), and whether it is the current session.",
      parameters: {},
      output: {
      ...
      presentCall: () => present("List live sessions", "read")
    })));
  } catch (error) {
