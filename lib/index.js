 ...
 *  - session_message_create(first_message?) — 创建新会话
 *
...
/**
 * 获取一个会话的标题：从 session/title 事件中提取。
 */
function findTitle(session) {
  const titleEvent = session.events.findLast((event) => event.type === "session/title");
  return titleEvent === undefined ? undefined : titleEvent.data.title;
}
