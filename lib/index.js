...
/** apply() 执行前必须已就绪的全局服务。部分为可选注入。 */
const inject = ["agents", "sessions", "tools", "sessionPersistence", "workspaceRegistry", "agentPresets"];
  ...
  try {
        ...
        if (target === undefined) {
          ...
          if (persistence !== undefined) {
            try {
              ...
              target = handle.agent;
              // 直接安装 model selection（不走 setup，避免 agentCtx.agent 未就绪的问题）
              const agentCtx = target.ctx;
              let picked;
              const selection = {
                get current() {
                  return picked ?? { provider: target.options.provider, model: target.options.model };
                },
                set current(next) { picked = next; },
                assembled: void 0
              };
              installModelSelection(agentCtx, selection);
              // 挂载该会话自己记录的 agent preset
              const presets = rootCtx.get("agentPresets");
              if (presets !== void 0) {
                const presetId = target.session?.header?.agentPreset;
                if (presetId) {
                  try {
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      async execute(_args, exec) {
        ...
        // 标题经 sessionTitle 服务读取（新版禁止直接同步读 session 事件）
        ...
        // 新版统一语料 API：一次给出「在线 + 已持久化」并自带 live 标记
        ...
        if (query !== undefined) {
          ...
          for (const record of records) {
            ...
            const target = rootCtx.agents.get(id);
            // 标题只在会话已加载（在线）时可得；持久化会话的标题需读整份日志，此处不取
            const title = live ? titleOf(rootCtx.sessions.get(id)) : undefined;
            result.push({
              session_id: id,
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          const buildSelection = (agentCtx) => {
            const newAgent = agentCtx.agent;
            if (newAgent === void 0) return undefined;
            let picked;
            return {
              get current() {
                return picked ?? { provider: newAgent.options.provider, model: newAgent.options.model };
              },
              set current(next) { picked = next; },
              assembled: void 0
            };
          ...
          const handle = await rootCtx.agents.create({
            ...
            setup: async (agentCtx) => {
              ...
              const selection = buildSelection(agentCtx);
